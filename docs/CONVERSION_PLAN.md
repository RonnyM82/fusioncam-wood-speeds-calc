...
| 6 | Publish and deploy, on Scott's word: the published package version swapped in, the baseline run against that build, the GitHub Pages workflow added and the Pages source switched, the first deploy, the baseline run against wood.fusioncam.co, and the Fusion add-in opened against the live site. Added at step 6 on Scott's ruling of 2026-09-24: the ball nose, bull nose and 3D surfacing work from commit bc85559 goes live behind a "beta tools" tick, off by default, on the page and in the panel; with it off both behave as the live site at commit 1e6c265 (CLAUDE.md, "The beta switch"). The baseline accepts the tick, and the shorter tool and diameter lists and the two plainer hints while it is off, as two ruled differences. | The live page reproduces the baseline; the panel receives its hello. DONE 2026-09-24, except Scott's own checks: live from `main` at `a77b008`; against wood.fusioncam.co the baseline was 52 identical with the accepted differences, the paint measurement passed, and the Fusion panel's harness rendered a job. Scott then checked the page on his phone and the Fusion add-in's palette against the live site, and both worked (2026-09-24), which completes the step and the conversion. |
