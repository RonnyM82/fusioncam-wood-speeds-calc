...
The public page is being rebuilt as a Vite and React app on the Livetools design system's parts, on the `react-conversion` branch ([docs/CONVERSION_PLAN.md](docs/CONVERSION_PLAN.md)). On that branch `index.html` and `src/` are the new page, `legacy.html` is the page as it was, unchanged, kept for comparison until the work is done, and `npm install` then `npm run check` runs the conformance check, the tests, the lint, the type check, the build and the paint measurement in one go. `npm install` also points git at the tracked pre-commit hook in `.githooks/`, which runs `npm run check` before every commit. `npm run dev` serves the new page, and `npm run build` then `npm run preview` serves the built site on port 4173, the Fusion panel included.

**The design system comes from npm.** `package.json` pins `@livetools/ui` at an exact published version. The paint measurement, the last step of `npm run check`, still reads the design system's paint probe from its repo checked out beside this one; `npm run check:site` is the same gate without it, and is what the GitHub Pages workflow in `.github/workflows/pages.yml` runs before it publishes. (This note lives here because `package.json` cannot carry a comment.)
